...
namespace BlobHashMaps
{
    ...
    public struct CellStateDataBlobMultiHashMap<TKey>
        where TKey : struct, IEquatable<TKey>
    {
        ...
        public bool TryGetFirstValue(TKey key, out CellStateData item, out CellStateDataBlobMultiHashMapIterator<TKey> it) =>
        ...
        public bool TryGetNextValue(out CellStateData item, ref CellStateDataBlobMultiHashMapIterator<TKey> it) =>
    ...
    public ref struct CellStateDataBlobBuilderMultiHashMap<TKey> 
        where TKey : struct, IEquatable<TKey>
    {
        internal CellStateDataBlobBuilderHashMapData<TKey> data;

        internal CellStateDataBlobBuilderMultiHashMap(int capacity, int bucketCapacityRatio, ref BlobBuilder blobBuilder, ref CellStateDataBlobHashMapData<TKey> data)
        {
#if BLOBHASHMAP_SAFE
            if (capacity <= 0)
        ...
        public void Add(TKey key, CellStateData item) => data.TryAdd(key, item, true);
        public int Capacity => data.keyCapacity;
        public int Count => data.Count;
    }
    
    public struct CellStateDataBlobMultiHashMapIterator<TKey>
        where TKey : struct, IEquatable<TKey>
    {
        internal TKey key;
