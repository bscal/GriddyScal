...
namespace BlobHashMaps.Data
{
    ...
    internal struct CellStateDataBlobHashMapData<TKey>
        where TKey : struct, IEquatable<TKey>
    {
        internal BlobArray<CellStateData> values;
        ...
        internal bool TryGetFirstValue(TKey key, out CellStateData item, out CellStateDataBlobMultiHashMapIterator<TKey> it)
        {
            it.key = key;
        ...
        internal bool TryGetNextValue(out CellStateData item, ref CellStateDataBlobMultiHashMapIterator<TKey> it)
        {
        ...
        internal NativeArray<CellStateData> GetValues(Allocator allocator)
        {
            int length = count[0];
            var arr = new NativeArray<CellStateData>(length, allocator, NativeArrayOptions.UninitializedMemory);
            for (int i = 0; i < length; i++)
                arr[i] = values[i];
            return arr;
        }
